    ...
    <section id="contact" className="gradient-dark py-14 md:py-20">
      <div className="container mx-auto">
        <div className="text-center mb-10 md:mb-12 scroll-animate">
          ...
          <h2 className="font-heading font-extrabold text-[28px] sm:text-[30px] md:text-[42px] text-primary-foreground">Get Your Free Quote</h2>
          <p className="text-primary-foreground/50 text-xs sm:text-sm mt-2">No obligation · We respond within 1 hour</p>
        </div>

        <div className="grid lg:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto">
          {/* Form */}
          <div className="scroll-animate bg-card rounded-3xl p-5 sm:p-7 md:p-9 shadow-[0_20px_80px_rgba(0,0,0,0.3)] order-2 lg:order-1">
            {submitted ? (
              ...
              <div className="space-y-3 sm:space-y-4">
                <div>
                  ...
                  <input name="name" value={formData.name} onChange={handleChange} placeholder="John Smith" className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all text-foreground text-sm min-h-[44px]" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    ...
                    <input name="phone" value={formData.phone} onChange={handleChange} placeholder="+64 XX XXX XXXX" className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all text-foreground text-sm min-h-[44px]" />
                  </div>
                  <div>
                    <label className="block text-navy font-medium text-sm mb-1">Email</label>
                    <input name="email" value={formData.email} onChange={handleChange} placeholder="john@example.com" className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all text-foreground text-sm min-h-[44px]" />
                  </div>
                </div>
                <div>
                  <label className="block text-navy font-medium text-sm mb-1">Service *</label>
                  <select name="service" value={formData.service} onChange={handleChange} className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-primary outline-none transition-all text-foreground bg-card text-sm min-h-[44px]">
                ...
                <div>
                  ...
                  <input name="suburb" value={formData.suburb} onChange={handleChange} placeholder="e.g. Petone, Lower Hutt" className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all text-foreground text-sm min-h-[44px]" />
                </div>
                <div>
                ...
                <button onClick={handleSubmit} className="w-full gradient-cta text-primary-foreground font-bold text-base py-3.5 rounded-xl hover:scale-[1.02] hover:shadow-lg transition-all shimmer min-h-[44px]">
                  Send Quote Request
                </button>
          ...
          <div className="space-y-4 order-1 lg:order-2">
            {[
              ...
              <div key={i} className="scroll-animate bg-primary-foreground/[0.06] border border-primary-foreground/10 rounded-2xl p-4 sm:p-5 flex gap-4 items-center hover:border-sky/40 transition-colors">
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-sky/15 flex items-center justify-center text-sky-accent shrink-0">{c.icon}</div>
                <div className="min-w-0">
                  <p className="text-sky-accent text-[10px] sm:text-[11px] font-semibold tracking-wider uppercase">{c.label}</p>
                  {c.href ? (
                    <a href={c.href} target="_blank" rel="noopener noreferrer" className="font-heading font-bold text-sm sm:text-base text-primary-foreground hover:text-sky transition-colors break-all">{c.value}</a>
                  ) : (
                    <p className="font-heading font-bold text-sm sm:text-base text-primary-foreground">{c.value}</p>
                  )}
            ...
            <a
              ...
              className="scroll-animate flex items-center justify-center gap-3 bg-whatsapp rounded-2xl px-6 py-3.5 w-full hover:brightness-110 hover:scale-[1.02] transition-all min-h-[44px]"
            >
              ...
              <div>
                <span className="text-primary-foreground font-bold text-sm sm:text-base block">Chat on WhatsApp</span>
                <span className="text-primary-foreground/70 text-xs">Tap to message us directly</span>
              </div>
            </a>

            {/* Hours */}
            <div className="scroll-animate bg-primary-foreground/5 border border-primary-foreground/[0.08] rounded-2xl p-4 sm:p-5">
