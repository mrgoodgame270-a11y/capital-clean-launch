      ...
      <div className="container mx-auto">
        <div className="grid lg:grid-cols-5 gap-12">
          <div className="lg:col-span-2 scroll-animate">
            ...
            <h2 className="font-heading font-extrabold text-[28px] md:text-[36px] text-navy leading-tight">Common Questions</h2>
            <p className="text-subtle text-sm mt-3">Can't find your answer?</p>
            <a
              href="https://wa.me/64XXXXXXXXX"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 mt-4 bg-whatsapp text-primary-foreground font-bold text-sm px-5 py-2.5 rounded-lg hover:brightness-110 transition-all"
            >
          ...
          <div className="lg:col-span-3 space-y-2.5">
            {faqs.map((faq, i) => (
              <div key={i} className={`scroll-animate bg-card rounded-xl border overflow-hidden transition-all ${openIndex === i ? 'border-l-[3px] border-l-sky border-sky/30' : 'border-border'}`}>
                <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full px-5 py-4 flex items-center justify-between text-left hover:bg-muted/50 transition-colors">
                  <span className={`font-heading font-semibold text-[14px] pr-4 ${openIndex === i ? 'text-primary' : 'text-navy'}`}>{faq.q}</span>
                  <span className="text-primary text-lg shrink-0">{openIndex === i ? '−' : '+'}</span>
                </button>
                <div className="overflow-hidden transition-all duration-300" style={{ maxHeight: openIndex === i ? '300px' : '0', padding: openIndex === i ? '0 20px 16px' : '0 20px' }}>
                  <p className="text-subtle text-[13px] leading-relaxed">{faq.a}</p>
                </div>
              </div>
            ))}
          </div>
